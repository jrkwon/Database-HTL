...
void addr2zip(void)
{
  ...
  hclrscr();
  hputsxy(20, 5, "         詬 簛潯 秮蚭ゅ选 纗嫛 詬");
  hputsxy(20, 7, "(暋) 妬(,妳) 暦(穝,) 宎弧爀 烦漛衋瑼禷");
  ...
  while( 1 ) {
    ...
    if(findzipcode(fp, addr, zipcode)) {
      ...
      hprintfxy(20, 20, "詢 秮蚭ゅ选揺 %s烦摗攁", zipcode);
    }
    else {
      hprintfxy(20, 20, "媋滃 簛礎暋 秮蚭ゅ选坅 范恆?");
    }
    hprintfxy(20, 25, "墎 衖宎禷 ?(Y/n)");
...
void zip2addr(void)
{
  ...
  hclrscr();
  hputsxy(20, 5, "         詬 秮蚭ゅ选潯 簛 纗嫛 詬");
  hputsxy(20, 7, "秮蚭ゅ选焛 烦漛衋 媋 簛焛 纗碼簛弧禷 ");
  ...
  while( 1 ) {
    ...
    if(findaddr(fp, zipcode, addr)) {
      hprintfxy(20, 20, "簛揺 %s烦摗攁", addr);
    }
    else {
      hprintfxy(20, 20, "媋滃 秮蚭ゅ选坅 瑏潯 瑮塿恆?");
    }
    hprintfxy(20, 25, "墎 衖宎禷 ?(Y/n)");
    if(toupper(getchext()) == 'N')
      break;
